...
public class Generator extends Configured implements Tool {
	...
	public static class Selector implements
		...
		private LongWritable genTime = new LongWritable(
		...
		public void configure(JobConf job) {
			...
			if (GENERATOR_COUNT_VALUE_DOMAIN.equals(job
					.get(GENERATOR_COUNT_MODE)))
				byDomain = true;
		...
		public void reduce(FloatWritable key, Iterator<SelectorEntry> values,
			...
			while (values.hasNext()) {
				...
				try {
					...
					reporter.getCounter("Generator", "MALFORMED_URL")
							.increment(1);
					continue;
				}
	...
	private Path partitionSegment(FileSystem fs, Path segmentsDir,
			Path inputDir, int numLists) throws IOException {
	...
	public int run(String[] args) throws Exception {
		...
		try {
			Path[] segs = generate(dbDir, segmentsDir, numFetchers, topN,
