...
public class CrawlDbReader implements Closeable {
		...
		protected static class LineRecordWriter implements
			...
			public synchronized void write(Text key, CrawlDatum value)
				...
				if (value.getMetaData() != null) {
					for (Entry<Writable, Writable> e : value.getMetaData()
							.entrySet()) {
	...
	public void processDBDumpJob(String crawlDb) throws Exception {
		if (LOG.isInfoEnabled()) {
		...
		MapFile.Reader[] readers = MapFileOutputFormat.getReaders(fs, new Path(
				crawlDb, CrawlDb.CURRENT_NAME), conf);
		...
		for (int i = 0; i < readers.length; i++) {
			...
			while (readers[i].next(aKey, value)) {
				CrawlDatum data = (CrawlDatum) value;
				String urlToSave = aKey.toString();
	...
	public static class CrawlDbDumpMapper implements
		...
		public void map(Text key, CrawlDatum value,
				...
				throws IOException {
			System.out.println("Retry" + retry + "Status" + status);
			// check retry
			if (retry != -1) {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 1) {
			System.err
					.println("Usage: CrawlDbReader <crawldb> (-stats | -dump <out_dir> | -topN <nnnn> <out_dir> [<min>] | -url <url> | -dbDump)");
			System.err
			...
			System.err
					.println("\t\t[-regex <expr>]\tfilter records with expression");
		...
		for (int i = 1; i < args.length; i++) {
			if (args[i].equals("-stats")) {
				...
				dbr.processDumpJob(crawlDb, param, conf, format, regex,
						status, retry);
			...
			} else if(args[i].equals("-dbDump")){
				dbr.processDBDumpJob(crawlDb);
			}else {
